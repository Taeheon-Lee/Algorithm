...
# 서로소 집합알고리즘은 Disjoint Set을 표현할 떄 사용하는 알고리즘으로 트리 구조를 활용
...
# 1. Make-Set(x) - 초기화 (n개의 원소가 개별 집합으로 이뤄지도록 초기화)
# 2. Find-Set(x) - 여러 노드가 존재할 떄, 두 개의 노드를 선택해서, 현재 두 노드가 서로 같은 그래프에 속하는지 판별하기 위해, 각 그룹의 최상단 원소(루트 노드)를 확인
# 3. Union(x, y) - 두 개별 집합을 하나의 집합으로 합침, 두 트리를 하나의 트리로 만듬
